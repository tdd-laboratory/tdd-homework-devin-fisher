...
_date_non_iso8601_pat = _whole_word(r'\d{2} (Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec) \d{4}')
...
def dates_iso8601(text):
    '''Find tokens that begin that contain an iso8601 date.'''
    for match in _date_iso8601_pat.finditer(text):
        yield('date', match)

def dates_non_iso8601(text):
    '''Find tokens that begin that contain a non-iso8601 date.'''
    for match in _date_non_iso8601_pat.finditer(text):
        yield ('date', match)
